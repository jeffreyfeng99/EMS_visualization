...
void sendDataToOpenGL()
{
	...
	//outer
	ShapeData shape = ShapeGenerator::makeCylinder(toPixel(3.0f / 2.0f), toPixel(DIVISIONSIZE), 0.5f, 0.5f, 0.5f, 1.0f);
	...
	//inner
	shape = ShapeGenerator::makeCylinder(toPixel(2.0f / 2.0f), toPixel(DIVISIONSIZE), 1.0f, 1.0f, 1.0f, 1.0f);
	...
	arm *left, arm* right, float qValuesLeft[NUMOFQVALS], float qValuesRight[NUMOFQVALS])
{
	...
	for (int i = leftIndex; i < left->numDivisions; i++)
	{
		...
		{
			(*tubeDraw)(toPixel(left->x[i]) - toPixel(10.8f), toPixel(left->y[i]), toPixel(-left->z[i]),
	...
	for (int i = rightIndex; i < right->numDivisions; i++)
	{
		...
		{
			(*tubeDraw)(toPixel(right->x[i]) + toPixel(10.8f), toPixel(right->y[i]), toPixel(-right->z[i]),
